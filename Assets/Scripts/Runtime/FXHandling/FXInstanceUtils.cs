...
namespace Spectral.Runtime.FX.Handling
{
	public static class FXInstanceUtils
	{
		...
		/// </summary>
		public static void ExecuteFX(IEnumerable<FXObjectData> effectDataObjects, Transform parent, float multiplier = 1)
		{
			...
			{
				ExecuteFX(effectDataObject, parent, multiplier);
			}
		}
		...
		/// </summary>
		public static void ExecuteFX(FXObjectData[] effectDataObjects, out FXInstance[] createdEffects, Transform parent, float multiplier = 1)
		{
			if (effectDataObjects == null)
			{
			...
			{
				createdEffects[i] = ExecuteFX(effectDataObjects[i], parent, multiplier);
			}
		}
		...
		/// </summary>
		public static void ExecuteFX(FXObjectData[] effectDataObjects, ref List<FXInstance> referencedInstanceList, Transform parent, float multiplier = 1)
		{
			if (effectDataObjects == null)
			{
				return;
			...
			for (int i = 0; i < effectDataObjects.Length; i++)
			{
				referencedInstanceList.Add(ExecuteFX(effectDataObjects[i], parent, multiplier));
			}
		}
		...
		private static FXInstance ExecuteFX(FXObjectData effectData, Transform parent, float multiplier)
		{
			...
			return AffiliatedHandler[effectData.BaseFX.GetType()].InitialiseFX(effectData.BaseFX,
																				new FXInstanceData(parent, multiplier, initiationDelay, customPositionOffset,
