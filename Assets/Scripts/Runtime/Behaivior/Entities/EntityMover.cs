...
namespace Spectral.Behaviours.Entities
{
	public class EntityMover : SpectralMonoBehavior
	{
		...
		[SerializeField] private int spawnBodyPartCount = 1;

		protected bool alive;
		public GameObject Head { get; set; }
		public List<GameObject> TorsoParts { get; set; }
		...
		private void ForceSpawnSetup()
		{
			if (entitySettings)
			{
				if (spawnBodyPartCount >= (entitySettings.MinParts - 1))
				{
					EntityFactory.BuildEntityBody(this, entitySettings, spawnBodyPartCount, UnityEngine.Random.Range(0, 360));
				}
				else
				{
					gameObject.SetActive(false);
#if SPECTRAL_DEBUG
					throw new System.Exception("A scene injected Entity (" + name + ") had a spawn torso count which was below the minumum count of body parts allowed by the Settings... Disabling");
#endif
				}
			}
			else
